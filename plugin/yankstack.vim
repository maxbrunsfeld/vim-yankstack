...
function! s:move_stack_index(offset)
  ...
  elseif s:last_paste.index >= len(g:yanklist())
    let s:last_paste.index = len(g:yanklist())-1
  endif
...
function! s:yanklist_add(item)
  ...
  let s:yanklist = s:yanklist[: g:yanklist_size-1]
endfunction

function! s:paste_from_yanklist()
  let [save_register, save_autoindent] = [@@, &autoindent]
  let [@@, &autoindent] = [g:yanklist(s:last_paste.index), 0]
